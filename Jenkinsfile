pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'anu'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/softencloud/SonarQube-Project-Kastro.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Anu -Dsonar.projectKey=AnuKey -Dsonar.java.binaries=target"
                }
            }
        }
